...
class Labeller2(Labeller):
    ...
    def apply_tripple_barrier(
        ...
        if pt_sl[0] > 0:
            ...
            pt = pd.Series(
                data=[np.nan] * len(events.index), index=events.index
            )
        if pt_sl[1] > 0:
            sl = -pt_sl[1] * events_['trgt']
        else:
        ...
        for loc, t1 in events_['t1'].fillna(close.index[-1]).iteritems():
            df0 = close[loc: t1]                                       # path prices
            df0 = (df0 / close[loc] - 1) * events_.at[loc, 'side']     # path returns
            # plt.plot(df0, label='path returns')
            # plt.hlines(sl[loc], xmin=loc, xmax=t1, label='sl', color='r')
            # plt.hlines(pt[loc], xmin=loc, xmax=t1, label='pt', color='g')
            # plt.legend()
            # plt.show()
            out.loc[loc, 'sl'] = df0[df0 < sl[loc]].index.min()        # earlisest stop loss
            out.loc[loc, 'pt'] = df0[df0 > pt[loc]].index.min()        # earlisest profit taking
    ...
    def get_events_tripple_barrier(
        ...
        if side is None:
            ...
            side_, pt_sl_ = side.loc[trgt.index.intersection(side.index)], pt_sl[:2]
        events = pd.concat({'t1': t1, 'trgt': trgt, 'side': side_}, axis=1).dropna(subset=['trgt'])
        df0 = Labeller2.apply_tripple_barrier(close, events, pt_sl_, events.index)
    ...
    def get_bins(close: pd.Series, events: pd.DataFrame, t1=False) -> pd.DataFrame:
        ...
        if 'side' in events_:
            ...
            if t1 is not None:
                ...
                # display(t1)
                vertical_first_touch_idx = events_[
                    events_['t1'] >= t1.loc[events_.index.tolist()]
                ].index
                # display(events_['t1'].isin(t1.values))
                # display(vertical_first_touch_idx)
                out.loc[vertical_first_touch_idx, 'bin'] = 0
        return out
    ...
    def add_vertical_barrier(
        ...
        t1 = pd.Series(tEvents.tolist()) + pd.Timedelta(days=numDays)
        t1 = pd.Series(t1.values, index=tEvents)
        # t1 = t1[t1 < close.shape[0]]
        return t1 #pd.Series(close.index[t1], index=tEvents[:t1.shape[0]])  # adding NaNs to the end

    def label_binarizer(self, events: pd.DataFrame = None) -> pd.DataFrame:
        if events is None:
            events = self.series.index
        t1 = self.add_vertical_barrier(self.series, events, numDays=15)
        events_ = self.get_events_tripple_barrier(
            close=self.series, tEvents=events, pt_sl=[1, 1],
            trgt=self.get_vol(self.series)*2, minRet=0,
            numThreads=1, t1=t1
        )
        labels = self.get_bins(close=self.series, events=events_, t1=t1)
        return labels